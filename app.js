const FOCUS={"the-invitation": "50% 44%", "dhol-comes-first": "48% 41%", "the-turn": "52% 47%", "the-step": "58% 48%", "the-circle": "47% 42%", "the-center": "50% 53%", "aajrakh": "48% 38%", "blurr": "48% 46%", "rny00498": "54% 47%", "installation-1": "47% 44%", "installation-2": "46% 46%", "installation-3": "47% 48%", "installation-4": "48% 52%"};
/* Where the subject actually is in each photograph, so the gallery's cover
   crop keeps it rather than anchoring to the middle of the frame. */
const photographs=[['the-invitation','The Invitation'],['dhol-comes-first','Dhol Comes First'],['the-turn','The Turn'],['the-step','The Step'],['the-circle','The Circle'],['the-center','The Center'],['aajrakh','Aajrakh'],['blurr','Blurr'],['rny00498','The Ground'],['installation-1','Under the lights'],['installation-2','The illuminated entrance'],['installation-3','Bamboo and tradition'],['installation-4','The fabric installation']];
const photoPath=file=>'assets/'+file+'.webp';
const track=document.querySelector('#gallery-track'),lightbox=document.querySelector('#lightbox');let photoIndex=0;
photographs.forEach(([file,title],i)=>{const button=document.createElement('button');button.className='gallery-card';button.setAttribute('aria-label','View '+title);button.innerHTML=`<div class="photo"><img src="${photoPath(file)}" alt="${title} — Divi Garba" loading="lazy" style="object-position:${FOCUS[file]}"></div><div class="caption"><span>${String(i+1).padStart(2,'0')}</span><h3>${title}</h3></div>`;button.addEventListener('click',()=>{showPhoto(i);lightbox.showModal()});track.append(button)});
function showPhoto(i){photoIndex=(i+photographs.length)%photographs.length;const[file,title]=photographs[photoIndex];document.querySelector('#light-img').src=photoPath(file);document.querySelector('#light-img').alt=title+' — Divi Garba';document.querySelector('#light-caption').textContent=title;for(const offset of [-1,1]){const preload=new Image();preload.src=photoPath(photographs[(photoIndex+offset+photographs.length)%photographs.length][0])}}
...
const terms=document.querySelector('#terms');function checkTerms(){if(location.hash==='#terms'&&!terms.open)terms.showModal();else if(location.hash!=='#terms'&&terms.open)terms.close()}window.addEventListener('hashchange',checkTerms);terms.addEventListener('close',()=>{if(location.hash==='#terms')history.replaceState(null,'',location.pathname+location.search)});checkTerms();
...
// Read the resting bounds once, avoiding pointer feedback from transformed geometry.
const depthResets=[];
