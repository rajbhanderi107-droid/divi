...
photographs.forEach(([file,title],i)=>{const button=document.createElement('button');button.className='gallery-card';button.setAttribute('aria-label','View '+title);button.innerHTML=`<div class="photo"><img src="${photoPath(file)}" alt="${title} — Divi Garba" loading="lazy"></div><div class="caption"><span>${String(i+1).padStart(2,'0')}</span><h3>${title}</h3></div>`;button.addEventListener('click',()=>{showPhoto(i);lightbox.showModal()});track.append(button)});
